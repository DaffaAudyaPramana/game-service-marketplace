...
export default function SuccessPage() {
  ...
  const handleCopy = async (text: string, label: string) => {
    try {
      ...
      console.error(err);
      alert(err instanceof Error ? err.message : "Gagal copy");
    }
  };
  ...
  const handleUpload = async () => {
    if (!orderId) {
      showToast("error", "Order ID tidak valid");
      return;
    }
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      try {
        ...
        if (!res.ok) {
          console.error("Gagal ambil order:", res.status, data);
          ...
          setError(data?.error || data?.message || "Gagal mengambil data order.");
          return;
        }
    ...
    <main className="bg-black text-white min-h-screen px-6 py-12">
      <div className="max-w-2xl mx-auto text-center">
        ...
        <div className="p-6 border border-white/10 rounded-xl mb-8">
          ...
          <p className="text-xl font-bold text-lime-400">{order.orderId}</p>

          <p className="mt-3 text-gray-400 text-sm">Total Pembayaran</p>
        ...
        <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] shadow-2xl">
          ...
          <div className="border-b border-white/10 bg-white/[0.02] p-5 text-left">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-bold text-white">Upload Bukti Transfer</h2>
